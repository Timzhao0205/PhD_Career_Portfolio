...
The F06I package originally vendored a full copy of its predecessor,
`02_Startup/06_Frontier_Idea_Research_2026-07`, under `src/06/`. All 405 files
were byte-identical to that folder, so the copies were removed during the
2026-08 reorganisation and replaced by `SNAPSHOT_MANIFEST.csv`, which records
`snapshot_path -> canonical_path` for each removed file.

Usage (from anywhere):

    python3 rehydrate.py            # restore every removed file
    python3 rehydrate.py --check    # report only; write nothing
    python3 rehydrate.py --subset 10_SOURCE_ATLAS  # restore one subtree
