...
- `CLAUDE.md` + `.claude/` — Claude Code memory, Sonnet/high interactive
  default, slash commands (`/log`, `/specs`, `/deep`), and the
  `rsi-editor` review agent. The autonomous mission in folder `06` uses
  its own explicit Fable/Sonnet stage allocation. Launch `claude` from
