...
for i = 1:length(files)
    if contains(files(i).name, 'scope_61')  || contains(files(i).name, 'scope_69') || contains(files(i).name, 'scope_70')
        ...
        xlabel('Frequency (Hz)');
        ylabel('Magnitude');
        title(["One Tesla QHS Mode FFT, V_{bias} = 0.4 V"]);
        xlim([24 5500]);
...
% add legend
legend({'Shot 62', 'Shot 66', 'Shot 67'}, 'Location', 'Best');
