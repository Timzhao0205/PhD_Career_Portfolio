...
ax.LineWidth = 1;
ax.FontSize = 11;
% I want text to be slightly bolder
ax.TickDir = 'in';     % or 'in' if you prefer
ax.XMinorTick = 'off';
ax.YMinorTick = 'off';
% grid on;
% grid minor
...
for i = 1:length(files)
    ...
    if contains(files(i).name, 'scope_41') 
        % read the data from the csv file, read more decimal places
        opts = detectImportOptions(files(i).name, 'NumHeaderLines', 0);
        opts = setvartype(opts, 'double');
        opts = setvaropts(opts, 'DecimalSeparator', '.');
...
for i = 1:length(files)
    % if contains(files(i).name, 'scope_66') || contains(files(i).name, 'scope_72')
    if contains(files(i).name, 'scope_72') 
        % read the data from the csv file, read more decimal places
        opts = detectImportOptions(files(i).name, 'NumHeaderLines', 0);
        opts = setvartype(opts, 'double');
        opts = setvaropts(opts, 'DecimalSeparator', '.');
...
legend('Plasma Discharge', 'Coil-Only Shot', 'Location', 'northwest');
