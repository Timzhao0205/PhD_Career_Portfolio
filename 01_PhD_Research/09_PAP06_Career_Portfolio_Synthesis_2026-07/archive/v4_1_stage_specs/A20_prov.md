...
- extracted old Folder 06 under `src/06o`
- `state/SOURCE_RENAMES.json` (historical CLAUDE/AGENTS files are available at
  their recorded inert working paths, such as `_claude_source` directories and
  filenames ending in `.source.txt`)
- `inputs/history.md`
- input SHA and duplicate facts in `INPUT_PROVENANCE.md`
- A10 only for its immutable hash/checkpoint; do not use its ranking to infer
