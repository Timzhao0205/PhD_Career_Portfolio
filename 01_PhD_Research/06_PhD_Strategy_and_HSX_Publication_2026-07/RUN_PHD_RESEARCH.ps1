...
<#
Claude-only, checkpointed runner for the July 2026 PhD/HSX mission.

Normal run and resume:
  powershell -ExecutionPolicy Bypass -File .\RUN_PHD_RESEARCH.ps1 -Resume

Explicitly start a new Claude retry cycle after a saved handoff:
  powershell -ExecutionPolicy Bypass -File .\RUN_PHD_RESEARCH.ps1 -Resume -RetryClaudeAfterHandoff
...
the live stream, event index, attempt state, and ChatGPT handoff snapshot are
flushed inside the project as work proceeds.
#>
[CmdletBinding()]
param(
    [switch]$Resume,
...
$Stages = [ordered]@{
    '00_inventory' = @{
        Prompt='prompts\00_inventory.md'; Model='sonnet'; Effort='high'; MaxTurns=70; Next='10a_literature_gan'
        ChatGptModel='gpt-5.6-sol'; ChatGptEffort='High'
        Expected=@(
            'outputs\00_INPUT_INVENTORY.md',
            'outputs\00_REQUIREMENTS_TRACE.csv',
            'outputs\00_CONFLICT_LEDGER.md',
            'outputs\00_CLAIM_BASELINE.csv'
        )
    }
    '10a_literature_gan' = @{
        Prompt='prompts\10a_literature_gan.md'; Model='sonnet'; Effort='xhigh'; MaxTurns=150; Next='10b_literature_fusion'
        ChatGptModel='gpt-5.6-sol'; ChatGptEffort='Extra High'
        Expected=@('evidence\10A_GAN_WBG_SOURCES.csv','evidence\10A_SYNTHESIS.md')
    }
    '10b_literature_fusion' = @{
        Prompt='prompts\10b_literature_fusion.md'; Model='sonnet'; Effort='xhigh'; MaxTurns=150; Next='10c_literature_methods'
        ChatGptModel='gpt-5.6-sol'; ChatGptEffort='Extra High'
        Expected=@('evidence\10B_FUSION_DIAGNOSTICS_SOURCES.csv','evidence\10B_SYNTHESIS.md')
    }
    '10c_literature_methods' = @{
        Prompt='prompts\10c_literature_methods.md'; Model='sonnet'; Effort='xhigh'; MaxTurns=150; Next='10d_literature_merge'
        ChatGptModel='gpt-5.6-sol'; ChatGptEffort='Extra High'
        Expected=@('evidence\10C_METHODS_SOURCES.csv','evidence\10C_SYNTHESIS.md')
    }
    '10d_literature_merge' = @{
        Prompt='prompts\10d_literature_merge.md'; Model='fable'; Effort='xhigh'; MaxTurns=150; Next='20_direction'
        ChatGptModel='gpt-5.6-sol'; ChatGptEffort='Max'
        Expected=@(
            'outputs\01_SOURCE_LEDGER.csv',
            'outputs\01_LITERATURE_REVIEW.md',
            'outputs\01_EVIDENCE_MAP.csv',
            'outputs\01_SOURCE_COVERAGE.md'
        )
    }
    '20_direction' = @{
        Prompt='prompts\20_direction.md'; Model='fable'; Effort='xhigh'; MaxTurns=90; Next='30_manuscript'
        ChatGptModel='gpt-5.6-sol'; ChatGptEffort='Max'
        Expected=@('outputs\02_RESEARCH_DIRECTION_DECISION.md','outputs\02_DIRECTION_SCORECARD.csv')
    }
    '30_manuscript' = @{
        Prompt='prompts\30_manuscript.md'; Model='fable'; Effort='xhigh'; MaxTurns=110; Next='40_experiment'
        ChatGptModel='gpt-5.6-sol'; ChatGptEffort='Max'
        Expected=@(
            'outputs\03_MANUSCRIPT_DIAGNOSIS.md',
            'outputs\03_REVIEWER_RESPONSE_MATRIX.csv',
            'outputs\03_PUBLICATION_ROUTE_DECISION.md'
        )
    }
    '40_experiment' = @{
        Prompt='prompts\40_experiment.md'; Model='fable'; Effort='xhigh'; MaxTurns=120; Next='50_patent'
        ChatGptModel='gpt-5.6-sol'; ChatGptEffort='Max'
        Expected=@(
            'outputs\04_HSX_EXPERIMENT_PLAN.md',
            'outputs\04_MEASUREMENT_REQUIREMENTS.csv',
            'outputs\04_DATA_ANALYSIS_PLAN.md',
            'outputs\04_UNCERTAINTY_AND_STATISTICS_PLAN.md'
        )
    }
    '50_patent' = @{
        Prompt='prompts\50_patent.md'; Model='fable'; Effort='xhigh'; MaxTurns=120; Next='60_timeline'
        ChatGptModel='gpt-5.6-sol'; ChatGptEffort='Max'
        Expected=@(
            'outputs\05_CANDIDATE_PROTECTABLE_CONCEPTS.md',
            'outputs\05_PRIOR_ART_LEDGER.csv',
            'outputs\05_DISCLOSURE_HOLD_CHECKLIST.md'
        )
    }
    '60_timeline' = @{
        Prompt='prompts\60_timeline.md'; Model='sonnet'; Effort='high'; MaxTurns=90; Next='70_redteam'
        ChatGptModel='gpt-5.6-sol'; ChatGptEffort='High'
        Expected=@(
            'outputs\06_24_MONTH_PHD_ROADMAP.md',
            'outputs\06_MILESTONES.csv',
            'outputs\06_STARTUP_READINESS.md',
...
function Set-AttemptState {
    ...
    $value = [ordered]@{
        ...
        chatgpt_windows_equivalent=[ordered]@{
            model=[string]$cfg.ChatGptModel
            effort=[string]$cfg.ChatGptEffort
            max_unavailable_fallback='Extra High'
        }
        note=$Note
...
function Write-HandoffState {
    ...
    if (-not [string]::IsNullOrWhiteSpace($Stage) -and $Stages.Contains($Stage)) {
        ...
        $chatGptModel = [string]$cfg.ChatGptModel
        $chatGptEffort = [string]$cfg.ChatGptEffort
        $expected = @($cfg.Expected)
    }
    $progress = Get-Progress
    $activeRoute = if ($Status -eq 'CHATGPT_HANDOFF_REQUIRED') {
        'chatgpt_windows_pending'
    } else {
    ...
    $state = [ordered]@{
        ...
        flags=[ordered]@{
            ...
            chatgpt_handoff_required=($Status -eq 'CHATGPT_HANDOFF_REQUIRED')
        ...
        chatgpt_windows=[ordered]@{
            model=$chatGptModel
            effort=$chatGptEffort
            if_max_is_unavailable='Extra High'
            open_as_primary_folder='01_PhD_Research'
            instructions='AGENTS.md'
            start_prompt='CHATGPT_WINDOWS_START_PROMPT.md'
            continuation_guide='CHATGPT_WINDOWS_CONTINUE.md'
        }
        session_id=$SessionId
        next_action=$NextAction
    }
    ...
    $markdown = @"
# ChatGPT Windows handoff snapshot
...
- Recommended ChatGPT model / effort: $chatGptModel / $chatGptEffort
- If Max is unavailable: use Extra High

## Continue

$NextAction

Open the extracted 01_PhD_Research folder as the primary local project in the
ChatGPT desktop app for Windows. Read AGENTS.md, then paste
CHATGPT_WINDOWS_START_PROMPT.md. The detailed machine-readable state is
...
function Resolve-ModelIntegrityAction([int]$CycleAttempt) {
    ...
    return 'SAVE_AND_HANDOFF_TO_CHATGPT_WINDOWS'
}
...
function Invoke-ClaudeAttempt {
    ...
    $shared = Get-Content -Raw -LiteralPath $SharedPromptPath
    $stagePrompt = Get-Content -Raw -LiteralPath (Join-Path $MissionRoot $cfg.Prompt)
    $prefix = ''
    if ($RetryCycle -gt 0) {
        $prefix = @"
...
"@
    ...
    Write-HandoffState -Status 'CLAUDE_IN_PROGRESS' -Stage $Stage -Attempt $Attempt `
        ...
        -NextAction 'Claude is active. If PowerShell is stopped manually, open this folder in ChatGPT Windows and continue from the durable files.' `
...
function Test-FinalLedger([string]$Path) {
    $basic = Test-CsvBatch -Path $Path -Minimum 150
    if (-not $basic.Ok) { return $basic }
    $rows = @(Import-Csv -LiteralPath $Path)
    return [pscustomobject]@{
        Ok=$true
        Message=("{0} total rows; final 150-source, metadata, type, access, identity, and uniqueness gates passed" -f
            @($rows).Count)
    }
}
...
function Test-StageOutputs([string]$Stage) {
    ...
    switch ($Stage) {
        '10a_literature_gan' {
            return (Test-CsvBatch -Path (Join-Path $MissionRoot 'evidence\10A_GAN_WBG_SOURCES.csv') -Minimum 55)
        }
        '10b_literature_fusion' {
            return (Test-CsvBatch -Path (Join-Path $MissionRoot 'evidence\10B_FUSION_DIAGNOSTICS_SOURCES.csv') -Minimum 55)
        }
        '10c_literature_methods' {
            return (Test-CsvBatch -Path (Join-Path $MissionRoot 'evidence\10C_METHODS_SOURCES.csv') -Minimum 55)
        }
        '10d_literature_merge' {
...
function Test-PackageBaseline {
    $required = @(
        ...
        'ENVIRONMENT_SETUP_MANIFEST.md','INPUT_CHECKSUMS.sha256',
        'TEST_CLAUDE_MODELS.ps1','INVOKE_CLAUDE_CHILD.ps1',
        'inputs\ORIGINAL_REQUEST.txt','inputs\ORIGINAL_REQUEST_AND_SETUP.md',
    ...
    if (-not (Test-Path -LiteralPath (
        Join-Path $ResearchRoot '01_Publications\submitted\regular_lsens\regular_lsens.tex'
    ) -PathType Leaf)) {
        throw 'Extracted regular_lsens manuscript source is missing.'
    }
    if (-not (Test-Path -LiteralPath (
...
function Invoke-Preflight {
    ...
    Write-Host 'PASS: package structure, PowerShell, JSON, checksums, source, and HSX inputs validated.' -ForegroundColor Green
    if ($DryRun) { return }

    $claudeCommand = Get-Command claude -ErrorAction SilentlyContinue
    if ($null -eq $claudeCommand) {
        throw 'Claude Code is not on PATH. Install or restore Claude Code, or continue through the included ChatGPT Windows handoff.'
    }
...
function Invoke-PolicySelfTest {
    ...
    if ((Resolve-ModelIntegrityAction 2) -ne 'SAVE_AND_HANDOFF_TO_CHATGPT_WINDOWS') {
    ...
    foreach ($name in @($Stages.Keys)) {
        ...
        if ([string]$cfg.ChatGptModel -ne 'gpt-5.6-sol') {
            throw "Stage $name lacks the documented ChatGPT Windows equivalent."
        }
    }
    ...
    Write-Host 'PASS: Fable must produce the accepted result; first failure retries in safe mode and second saves the ChatGPT Windows handoff.' -ForegroundColor Green
    Write-Host 'PASS: all 12 stages are Claude-only and carry a GPT-5.6 Sol continuation mapping.' -ForegroundColor Green
}
...
try {
    ...
    foreach ($stage in @($Stages.Keys)) {
        ...
        Write-Host ("{0,-25} claude={1}/{2} chatgpt={3}/{4} {5}" -f
            $stage,$cfg.Model,$cfg.Effort,$cfg.ChatGptModel,$cfg.ChatGptEffort,$action)
    }
    ...
    $pendingHandoff = (
        ...
        [string]$pendingState.status -eq 'CHATGPT_HANDOFF_REQUIRED'
    )
    if (-not $pendingHandoff) {
        Write-HandoffState -Status 'READY_IF_NEEDED' `
            -NextAction 'Run Claude with RUN_EVERYTHING.ps1, or open the project in ChatGPT Windows if Claude cannot continue.'
    ...
    foreach ($stage in @($Stages.Keys)) {
        ...
        if ($Resume -and $null -ne $attemptState) {
            ...
            if ($savedStatus -eq 'CHATGPT_HANDOFF_REQUIRED') {
                if (-not $RetryClaudeAfterHandoff) {
                    Write-HandoffState -Status 'CHATGPT_HANDOFF_REQUIRED' -Stage $stage `
                        ...
                        -NextAction 'Open the project in ChatGPT Windows and paste CHATGPT_WINDOWS_START_PROMPT.md, or rerun with -RetryClaudeAfterHandoff after confirming Claude usage is available.' `
                        -RetryCycle $retryCycle -CycleAttempt $cycleAttempt `
                        -HandoffHistory $handoffHistory
                    Write-Warning 'This stage is checkpointed for ChatGPT Windows handoff. Claude was not restarted because no explicit retry switch was supplied.'
                    $stopForHandoff = $true
                    break
                }
            ...
            if ($savedStatus -eq 'MODEL_RETRY_PENDING') {
                ...
                if ($savedStatus -ne 'CHATGPT_HANDOFF_REQUIRED') {
                    $attempt = Get-IntegerProperty -InputObject $attemptState `
                        -Name 'attempt' -Default 1
        ...
        while (-not $stageComplete -and $cycleAttempt -le 2) {
            ...
            $integrityNextAction = if ($trackedDowngrade) {
                ...
                'SAVE_FOR_CHATGPT_WINDOWS_CONTINUATION'
            }
            $modelEvidenceNote = (
            ...
            if ($trackedDowngrade) {
                ...
                Set-AttemptState -Stage $stage -Attempt $attempt `
                    -Status 'CHATGPT_HANDOFF_REQUIRED' -StreamFile $result.StreamFile `
                ...
                Write-HandoffState -Status 'CHATGPT_HANDOFF_REQUIRED' -Stage $stage `
                    ...
                    -NextAction 'Stop Claude. Open the project in ChatGPT Windows, select the recommended GPT-5.6 Sol effort, and paste CHATGPT_WINDOWS_START_PROMPT.md. If you later explicitly authorize another Claude cycle, rerun with -RetryClaudeAfterHandoff.' `
                    -RetryCycle $retryCycle -CycleAttempt $cycleAttempt `
                    -HandoffHistory $handoffHistory
                Add-OperationRecord -Stage $stage -Operation 'second_downgrade_handoff' `
                    -Attempt $attempt -Status 'CHATGPT_HANDOFF_REQUIRED' `
                    -RequestedModel ([string]$cfg.Model) `
                    -RequestedEffort ([string]$cfg.Effort) `
                    -ReportedModels $result.ModelsText -DowngradeFlag $true `
                    -DowngradeCount 2 -SessionId $result.SessionId `
                    -StreamFile $result.StreamFile -Checkpoint $HandoffJsonPath `
                    -NextAction 'Continue in ChatGPT Windows with the mapped GPT-5.6 Sol model and effort.' `
            ...
            if ($result.Integrity -ne 'ok') {
                ...
                Write-HandoffState -Status 'CHATGPT_HANDOFF_REQUIRED' -Stage $stage `
                    ...
                    -DowngradeCount $downgradeCount `
                    -GeneratedPrompt $result.GeneratedPrompt `
                    -NextAction 'Open the project in ChatGPT Windows and continue from the saved prompt, stream, logs, and partial files, or explicitly start a fresh Claude retry with -RetryClaudeAfterHandoff.' `
                    -RetryCycle $retryCycle -CycleAttempt $cycleAttempt `
                    -HandoffHistory $handoffHistory
                Add-OperationRecord -Stage $stage -Operation 'runtime_failure_handoff' `
                    ...
                    -StreamFile $result.StreamFile -Checkpoint $HandoffJsonPath `
                    -NextAction 'Continue in ChatGPT Windows or explicitly authorize a fresh Claude retry.' `
                    -Notes ("{0};retry_cycle={1};cycle_attempt={2};{3}" -f
                        $result.ErrorText,$retryCycle,$cycleAttempt,$modelEvidenceNote)
                $stopForHandoff = $true
                break
            ...
            if (-not $validation.Ok) {
                ...
                Write-HandoffState -Status 'CHATGPT_HANDOFF_REQUIRED' -Stage $stage `
                    ...
                    -DowngradeCount $downgradeCount `
                    -GeneratedPrompt $result.GeneratedPrompt `
                    -NextAction 'Open the project in ChatGPT Windows, repair only the failed acceptance gate, revalidate, and continue; or explicitly start a fresh Claude retry with -RetryClaudeAfterHandoff.' `
                    -RetryCycle $retryCycle -CycleAttempt $cycleAttempt `
                    -HandoffHistory $handoffHistory
                Write-ModelRecord -Stage $stage -Attempt $attempt `
                    -RequestedModel ([string]$cfg.Model) `
                    ...
                    -Notes ("ChatGPT Windows handoff prepared;{0}" -f
                        $modelEvidenceNote)
                Add-OperationRecord -Stage $stage -Operation 'validation_failed_handoff' `
                    ...
                    -SessionId $result.SessionId `
                    -StreamFile $result.StreamFile -Checkpoint $HandoffJsonPath `
                    -NextAction 'Repair and validate in ChatGPT Windows.' `
                    -Notes ("{0};retry_cycle={1};cycle_attempt={2};{3}" -f
    ...
    if ($stopForHandoff) {
        Write-Section 'Saved for ChatGPT Windows continuation'
        Write-Host 'Claude was not routed to another provider. All current state is inside this folder.' -ForegroundColor Yellow
        Write-Host 'Open CHATGPT_WINDOWS_CONTINUE.md and paste CHATGPT_WINDOWS_START_PROMPT.md in the ChatGPT desktop app.' -ForegroundColor Yellow
        exit 2
    }
    ...
    if ($allComplete) {
        Write-HandoffState -Status 'MISSION_COMPLETE' `
            -NextAction 'Review outputs\FINAL_EXECUTIVE_STRATEGY.md and outputs\FINAL_ACTION_PLAN.md.'
    ...
    try {
        ...
        Add-OperationRecord -Stage $script:CurrentStage -Operation 'launcher_fatal' `
            -Attempt $script:CurrentAttempt -Status 'CHATGPT_HANDOFF_REQUIRED' `
            -StreamFile $script:CurrentStream -Checkpoint $HandoffJsonPath `
            -NextAction 'Continue in ChatGPT Windows or repair the named prerequisite.' `
        ...
        Write-HandoffState -Status 'CHATGPT_HANDOFF_REQUIRED' `
            -Stage $script:CurrentStage -Attempt $script:CurrentAttempt `
            -Reason 'LAUNCHER_FATAL' -StreamFile $script:CurrentStream `
            -NextAction 'Open the project in ChatGPT Windows and continue from durable files, repair the displayed prerequisite, or explicitly retry Claude with -RetryClaudeAfterHandoff.' `
