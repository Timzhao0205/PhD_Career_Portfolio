...
fig_shot21 = figure('Color', 'w', 'Position', [100, 100, 1400, 450]);
% figWidth = 1200; 
% figHeight = 900;
...
xlim([0.83 0.89]);
ylim([0 0.6]);set(gca, 'FontSize', 12);
...
plot(x2, y2, 'linewidth', 1.5, 'Color', [0.8500, 0.3250, 0.0980]);
xlim([0.8 0.86]);
ylim([-5 65]);
% grid on;
% grid minor;
% title('Shot 21');
xlabel('Time (s)');
ylabel('Stored Energy (J)');set(gca, 'FontSize', 12);
