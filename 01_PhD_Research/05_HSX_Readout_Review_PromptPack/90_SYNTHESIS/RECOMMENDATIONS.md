...
| Q5 | Which flange for 12 conductors? | **Accu-Glass 9C2-275 (P/N 100012)** — 18-pin (2× 9-pin Sub-C) on **2.75″ CF**, $685; meets/exceeds all 9D-275 ratings (500 VDC family-confirmed, 5 A/pin, −200…250 °C, 1×10⁻¹⁰ Torr). Key finding: **no ≥12-pin Sub-D exists on 2.75″ CF** — the Sub-C family keeps the existing port. System ≈ $1,572 (2× 100040 vacuum-side PEEK/Kapton assemblies, 2× 100020 shielded air-side). Runner-up: 15D-450 (single 15-pin Sub-D, 4.5″ CF, system ≈ $952) if UW confirms a 4.5″ port; then 25D-450 (+$32) is the better buy. | **CHANGE (9D-275 → 9C2-275)** | Medium-high (two purchase gates open) | ≈ $1,572 | `50_FLANGE/FLANGE_SELECTION.md` → **GATE 2** |
...
2× additional board builds (option C); 9C2-275 system purchase (after Gate 2 clears); LCC
...
2. **Now — one UW-Madison email + one Accu-Glass call** (details in `DECISION_GATES.md`):
   port CF size at the probe location; Sub-C tower protrusion height above the flange face;
   in-vessel temperature at the probe; bakeout plan; whether the Ø31.75 mm envelope derives
   from the port bore. These clear Gates 2 and 3 preconditions.
...
6. **Purchase the 9C2-275 system** once Gate 2's two conditions clear.
